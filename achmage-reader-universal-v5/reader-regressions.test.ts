...
describe('reader regressions', () => {
  it('keeps the ToC sync effect aligned with live article headings and explicit reveal events', () => {
    ...
    expect(source).toContain("scrollElementIntoViewWithOffset(document.getElementById(id) as HTMLElement, 'smooth');");
    expect(source).toContain("window.history.pushState(null, '', `#${encodeURIComponent(id)}`);");
    expect(source).not.toContain("target.scrollIntoView({behavior: 'smooth', block: 'start'})");
    expect(source).toContain("article.addEventListener('click', onArticleClick, true);");
    expect(source).toContain("article.removeEventListener('click', onArticleClick, true);");
    expect(source).toContain('void handleArticleLinkClick({');
    expect(source).toContain('window.__ACHMAGE_LINK_DEBUG__');
    expect(source).not.toContain('onClickCapture={onClickCapture}');
    expect(linkSource).toContain('export const findClosestArticleAnchor = (');
    expect(linkSource).toContain('export const handleArticleLinkClick = ({');
    ...
    expect(linkSource).toContain("const composedPath = typeof event.composedPath === 'function' ? event.composedPath() : undefined;");
  ...
  it('keeps anchor jumps below the sticky header and dark surfaces close to the cmdspace reference', () => {
    ...
    expect(tokensCss).toContain("--color-canvas: #020202;");
    expect(tokensCss).toContain("--color-surface: #060606;");
    expect(tokensCss).toContain("--color-surface-muted: #0b0b0b;");
    ...
    expect(baseCss).toContain(":root[data-theme='dark'] .home-hero {");
    expect(baseCss).toContain(":root[data-theme='dark'] .doc-header__standfirst {");
    expect(baseCss).toContain('background: var(--color-surface-muted);');
  });
});
