...
import {getBasename, normalizePathSeparators, stripExtension} from './path-utils';
...
export const createObsidianLinkLookup = (documents: SourceDocument[]): ObsidianLinkLookup => {
  ...
  for (const document of documents) {
    ...
    add(getBasename(document.filePath), entry);
    add(document.relativePath, entry);
    add(stripExtension(document.relativePath), entry);
...
const resolveWikiTarget = (
  ...
  return {
    url: `?view=reader&doc=${encodeURIComponent(entry.slug)}${anchor}`,
    label: entry.title,
    missing: false,
  };
};
...
export const OBSIDIAN_CALLOUT_META_MARKER = 'achmage-callout-meta';

const formatFallbackCalloutTitle = (type: string): string => {
  const raw = type.trim();
...
export const preprocessObsidianMarkdown = (
  ...
  const withCallouts = markdown.replace(calloutLinePattern, (_, type: string, title: string) => {
    ...
    const visibleTitle = title?.trim() ? title.trim() : formatFallbackCalloutTitle(trimmedType);
    const encodedType = encodeURIComponent(trimmedType);
    const encodedTitle = encodeURIComponent(visibleTitle);

    return `> **${visibleTitle}** <!--${OBSIDIAN_CALLOUT_MARKER}--> <!--${OBSIDIAN_CALLOUT_META_MARKER}:${encodedType}|${encodedTitle}-->`;
  });
