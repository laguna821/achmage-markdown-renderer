...
> [!IMPORTANT]
> 렌더러 기준 마크다운 노트 작성법 (빠른 실천 가이드)
> - 노트 모든 구간에 `제목1 (Heading 1 - #)` 사용 금지
> - 노트 모든 구간 제목은 `제목2 (Heading 2 - ##)` 및 `제목3 (Heading 3 - ###)` 만 사용할 것
>     - 가독성에 영향을 미치는 글 작성 순서는 다음과 같다:
>     - `제목2 (Heading 2 - ##) `
>     - `제목3 (Heading 3 - ###)`
>     - Quote, Note 등 `노트 표시` (**각 소제목 섹션 처음 또는 끝에 한 번 삽입**할 것을 추천) 
>     - `본문` 글 (bulletpoint 없음)
>     - 주요 포인트를 정리한 `bulletpoint (-) 및 indentation`
>     - 본문 글 중간에 적절히 `CMDS Eagle` 로 `img hippo 링크`로 삽입한 `스크린샷` 혹은 `유튜브 링크`
>     - `Mermaid diagram` 으로 도표 등을 그린 것
>     - 마크다운 `테이블`
...
### 4.1 v4와 v5는 무엇이 다른가 (별로 알 필요 없는 TMI)

> [!NOTE]
> v4 (Node.js 기반 CMD 실행)
...
> [!NOTE]
> v5 (Tauri 기반 msi 인스톨러로 간편 설치 - 윈도우 기준)
...
> [!TIP]
> v4가 "잘 쓰는 사람이 직접 실행하는 툴"에 가까웠다면, 
> v5는 "배포 가능한 독립 실행 앱"에 가깝습니다.

## 5. v5 빠른 시작

> [!NOTE]
> Windows OS
> 1. [GitHub Releases](https://github.com/laguna821/achmage-markdown-renderer/releases/tag/v5.0.0-beta.22)에서 installer `.msi` 를 받습니다. (최신버전: 5.0.0-beta.22)
> 2. 설치형이면 설치 후 실행합니다.
...
> v5는 현재 윈도우 msi는 stable release로 새 컴퓨터에서 바로 실행됩니다. macOS 버전은 현재 패키지 수정 및 업데이트 중입니다.

## 6. 첫 실행에서 무엇을 하게 되나

앱을 처음 열면 가장 중요한 입력은 하나입니다.

> [!NOTE] 내 Obsidian vault 경로
> - 예시:
> ```text
...
앱은 그 폴더 안의 Markdown 문서를 읽어와 홈 화면과 `reader`를 화면에 반영합니다.
...
### 8.4. Omnisearch가 들어 있다

홈 화면에서 제목만 찾는 것이 아니라 문서 전반을 검색할 수 있습니다.

> [!tip]
> Markdown renderer 검색 활용법
> 지원 예시: 
> - 제목 검색
> - 본문 검색
> - YAML 메타데이터 검색
...
### 8.5. 테마를 바꿔가며 같은 문서를 다르게 볼 수 있다
...
### 8.7. Markdown와 Obsidian 문법을 폭넓게 처리한다
...
## 10. Achmage Markdown Renderer 기준 Markdown 작성법
...
### 10.1 왜 이 렌더러에서는 잘 읽히는 글이 다르게 보이나
...
## 11. 가장 먼저 지켜야 할 Markdown-Renderer 기준 작성 구조
### 11.1 문서의 큰 구획은 `##`로 나눈다
...
**주의: 문서 본문에 `#`을 모든 구간에서 쓰지 마십시오 (본 렌더러는 `##`과 `###` 기준으로 최적화 되어 있습니다).**
...
> 프론트 매터에 `title` 항목을 반드시 포함해야 제대로 검색이 편하게 됩니다

## 13. 본문을 쓸 때의 실제 규칙
### 13.1 문단은 "한 문단 한 메시지"로 쓴다
...
Obsidian에 글만 쓰세요.
Achmage Markdown Renderer는 그 Markdown을 바로 보여주는 화면으로 바꾸는 프로젝트입니다. 이 프로젝트의 진짜 힘은 "Markdown을 렌더링한다"는 사실만이 아니라, 
**처음부터 렌더러 기준으로 잘 읽히는 문서를 쓰게 만든다**는 데 있습니다.
